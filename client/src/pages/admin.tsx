  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (currentUser) {
          // Optional: Check for admin role if your user object has it
           // if (currentUser.role !== 'admin') { ... handle unauthorized ... }
          setUser(currentUser);
          fetchData();
        } else {
          setUser(null);
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
        <div className="p-8 bg-white rounded-xl shadow-lg flex flex-col items-center max-w-md w-full text-center">
          ...
          <div className="flex flex-col space-y-3 w-full">
            <button
              onClick={() => setLocation("/login?mode=login")}
              className="w-full px-6 py-3 bg-gray-900 text-white rounded-lg shadow hover:bg-gray-800 transition-colors font-medium flex items-center justify-center"
            >
              Go to Login Page
            </button>
            <button
              onClick={() => setLocation("/")}
              className="w-full px-6 py-3 bg-white border border-gray-300 text-gray-700 rounded-lg shadow-sm hover:bg-gray-50 transition-colors font-medium"
            >
              Back to Home
            </button>
          </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
        <div>
          ...
          <div className="mt-2 text-sm text-gray-600 flex items-center">
            <span className="mr-2">Welcome, {user.displayName || user.username}</span>
