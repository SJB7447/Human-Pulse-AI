...
## 7) aiDraftOps 확인 및 우선순위
- 반영 상태: `aiDraftOps` 지표는 `/api/admin/stats` 응답과 관리자 대시보드에 이미 연결됨.
- 현재 상태: 파일 영속화(`server/data/ai_draft_ops_metrics.json`)를 적용해 서버 재시작 후에도 지표 복원 가능.
- 잔여 한계: 단일 인스턴스 파일 저장 방식이라 멀티 인스턴스/분산 환경 집계에는 한계가 있음.

### Priority
- P0 (완료): `aiDraftOps` DB/로그 영속화 1차
  - `admin_action_logs`에 `ai_draft_metric_v1` 이벤트를 적재.
  - 서버 시작 시 DB 로그 집계로 `aiDraftOps` 복원.
  - 파일 영속화는 DB 장애/테이블 미구성 상황의 fallback으로 유지.
- P1: 임계치 운영 설정화
  - similarity/schema/compliance 임계치를 환경/관리자 설정으로 외부화.
- P2: 대시보드 추세 시각화
  - 최근 7일/30일 성공률, 차단 유형 비율, 재시도율 차트 추가.
