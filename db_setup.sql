...
CREATE INDEX IF NOT EXISTS idx_article_reviews_updated_at ON article_reviews(updated_at DESC);
...
-- Policy: Article reviews are managed by service role (implicitly denied for anon)
-- (No explicit policy to keep admin review data private by default)
